# -*- coding: utf-8 -*-
"""
Created on Thu Nov  1 13:41:18 2018
...
def onMouse(event, x,y, flags, pararm):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        if not rect_over:
            print("select grap-cut with mouseleftbtn")
        else:
            drawing = True
...
def grabcut():
    ...
    print("오른쪽 마우스 누르고 영역을 지정한 후 n을 누르기")
    ...
    while True:
        ...
        if k == ord('0'):
            print("왼쪽 마우스로 제거할 부분 표시 후 n")
            value = DRAW_BG
            
        elif k == ord('1'):
            print("왼쪽 마우스로 복원할 부분을 표시 한 후 n")
            value = DRAW_FG
        elif k == ord('r'):
            print("reset")
            rect = (0,0,1,1)
            ...
            mask = np.zeros(img, shape[:2], dtype = np.uint8)
            output = np.zeros(img.shape, np.uint8)
            print("0: 배경제거 1: 복원전경 n:적용하기, r:리셋")
        elif k == ord('n'):
